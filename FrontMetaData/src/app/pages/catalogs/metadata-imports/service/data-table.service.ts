...
updateColumn(column: Column): Observable<any> {
  return this.http.put(`${this.baseUrl}/columns/${column.idColumn}`, column ,{
    headers: { 'Content-Type': 'application/json' }
});
}
updateName(tableId: number, name: string): Observable<any> {
  return this.http.post(`${this.baseUrl}/tables/${tableId}/name`, name, {
      headers: { 'Content-Type': 'application/json' }
  });
  
}
updateLabels(idColumn: number, labels: string[]): Observable<any> {
  return this.http.put(`${this.baseUrl}/columns/${idColumn}/labels`, labels);
}
